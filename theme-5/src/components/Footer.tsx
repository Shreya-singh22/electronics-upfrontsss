...
export default function Footer() {
  ...
  const brandName = customization?.brand?.name || "ELECTRO";
  const brandSuffix = customization?.brand?.suffix || "PULSE";
  const brandTagline = customization?.brand?.tagline || "Technology";
  const brandDescription = customization?.brand?.description || "Your destination for the most advanced gadgets and electronics. We set the pulse of modern technology.";
  const copyrightText = customization?.brand?.copyright || "© 2026 ElectroPulse Electronics. All rights reserved.";
  const brandLogo = customization?.brand?.logo || "/logo.png";
  return (
    <footer className="bg-card border-t border-border cursor-pointer" onClick={handleSectionClick}>
      {/* Features Bar */}
        ...
        <div className="space-y-6">
          <Link href="/" className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              <span className="font-heading font-black text-xl text-foreground tracking-tighter">{brandName}<span className="text-primary">{brandSuffix}</span></span>
